"""RegimeVector schema tests — validate the frozen-schema invariants.
...
def _valid_vector() -> RegimeVector:
    return RegimeVector(
...
def test_valid_vector_constructs():
    v = _valid_vector()
    assert v.pixels_on_target.p50_px == 42.0
    assert v.target_novelty.confidence is ProbeConfidence.LOW
    assert v.catalog_version == "v0-2026-09-17"
...
def test_missing_confidence_fails():
    ...
    with pytest.raises(ValidationError):
        PixelsOnTargetReading(p10_px=18.0, p50_px=42.0)  # type: ignore[call-arg]


def test_missing_axis_fails():
    with pytest.raises(ValidationError):
        RegimeVector(  # type: ignore[call-arg]
            pixels_on_target=PixelsOnTargetReading(
                p10_px=18.0, p50_px=42.0, confidence=ProbeConfidence.HIGH
            ),
...
def test_extra_field_forbidden():
    v = _valid_vector()
    with pytest.raises(ValidationError):
        v.model_copy(update={"unexpected_axis": 1.0}).model_validate(
            v.model_dump() | {"unexpected_axis": 1.0}
        )


def test_extra_field_on_reading_forbidden():
    with pytest.raises(ValidationError):
        PixelsOnTargetReading.model_validate(
...
def test_frozen():
    v = _valid_vector()
    with pytest.raises(ValidationError):
        v.pixels_on_target.p10_px = 999.0  # type: ignore[misc]


def test_negative_pixels_rejected():
    with pytest.raises(ValidationError):
        PixelsOnTargetReading(p10_px=-1.0, p50_px=42.0, confidence=ProbeConfidence.HIGH)
...
def test_catalog_version_required_and_nonempty():
    with pytest.raises(ValidationError):
        RegimeVector.model_validate(
            _valid_vector().model_dump() | {"catalog_version": ""}
        )
